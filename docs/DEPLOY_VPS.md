# Deploy do Agente CARNEIRO.IA na VPS (Hostinger / Ubuntu)
...
> O `.git` de producao e ISOLADO (so a pasta `ALVES_CARNEIRO_ADVOGADOS`). Nenhum segredo
...
> A VPS de producao do Alves Carneiro Advocacia e Consultoria Jurídica AINDA NAO foi provisionada.
...
- **Projeto na VPS:** `/home/deploy/carneiro-advogados` | virtualenv em `.venv`.
- **Clone do repo privado:** via **deploy key** read-only (`~/.ssh/github_deploy` na VPS).
- **Servico:** systemd `carneiro-agente` (enabled, Restart=always), uvicorn na porta **8787**.
...
```bash
# logs em tempo real
journalctl -u carneiro-agente -f
# status / restart
sudo systemctl status carneiro-agente
sudo systemctl restart carneiro-agente
# deploy de nova versao
cd /home/deploy/carneiro-advogados && git pull
...
- Repositorio no GitHub (privado) com o codigo — `pabadvogados-hub/carneiro-advogados`.
...
Repositorio **privado** ja criado: `pabadvogados-hub/carneiro-advogados`. Na pasta
do projeto:

```bash
git remote add origin git@github.com:pabadvogados-hub/carneiro-advogados.git
git push -u origin master
```

(Se usar HTTPS no lugar de SSH: `https://github.com/pabadvogados-hub/carneiro-advogados.git`
...
```bash
cd ~
git clone git@github.com:pabadvogados-hub/carneiro-advogados.git carneiro-advogados
cd carneiro-advogados
```

Provisione o sistema (Python, venv, tesseract, Chromium do Playwright):

```bash
bash deploy/setup_vps.sh ~/carneiro-advogados
```
...
```powershell
# rode na maquina LOCAL, dentro da pasta ALVES_CARNEIRO_ADVOGADOS
scp config/.env                 deploy@SEU_IP:~/carneiro-advogados/config/.env
scp config/credentials.json     deploy@SEU_IP:~/carneiro-advogados/config/credentials.json
scp config/oauth_credentials.json deploy@SEU_IP:~/carneiro-advogados/config/oauth_credentials.json
scp config/token.json           deploy@SEU_IP:~/carneiro-advogados/config/token.json
scp config/timbrado_modelo.docx deploy@SEU_IP:~/carneiro-advogados/config/timbrado_modelo.docx
```
...
sudo cp deploy/carneiro-agente.service /etc/systemd/system/carneiro-agente.service
# confira User= e os caminhos dentro do arquivo (default: usuario 'deploy', ~/carneiro-advogados)
sudo systemctl daemon-reload
sudo systemctl enable --now carneiro-agente
sudo systemctl status carneiro-agente --no-pager
```
...
Quando quiser expor com dominio e SSL (ex.: `agente.carneiroadvogados.com.br` —
