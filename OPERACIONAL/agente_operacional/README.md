# Agente Operacional ALVES_CARNEIRO_ADVOGADOS (CARNEIRO.IA)

Agente autonomo que recebe tarefas do ADVBOX (atribuidas ao usuario-agente **CARNEIRO.IA**) via webhook disparado pela N8N, produz o resultado (peca juridica, notificacao de cliente, resposta operacional) e devolve a tarefa para quem pediu com o link do documento.
...
Equipe do escritorio
   |
   v  (atribui tarefa a CARNEIRO.IA no ADVBOX)
...
AGENTE_OP_PORT=8787
ADVBOX_USER_AGENTE=<id do usuario-agente CARNEIRO.IA no ADVBOX>
ADVBOX_TASK_TYPE_ACOMPANHAMENTO=<id do tipo de tarefa para retornos>
ESCRITORIO_ASSINATURA=Dra. Fabiana Silva Alves Carneiro - OAB/RJ 188.762 - Rio de Janeiro/RJ
...
- **So CARNEIRO.IA pendente** -> trocar `SEU_ADVBOX_USER_AGENTE` pelo ID do usuario-agente no ADVBOX
...
- Retorno no ADVBOX sempre com `from = usuario-agente (CARNEIRO.IA)`
