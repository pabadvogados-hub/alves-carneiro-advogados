...
Usa a CARNEIRO.IA (Claude) para analisar documentos e gerar a peca completa.
"""
import anthropic
import json
import os
...
# ============================================================
# DADOS DO ESCRITORIO (lidos do ambiente, com defaults ALVES_CARNEIRO_ADVOGADOS)
# ============================================================
ESCRITORIO_NOME = os.getenv('ESCRITORIO_NOME', 'Alves Carneiro Advocacia e Consultoria Jurídica')
ESCRITORIO_ADVOGADO = os.getenv('ESCRITORIO_ADVOGADO', 'Dra. Fabiana Silva Alves Carneiro')
ESCRITORIO_OAB = os.getenv('ESCRITORIO_OAB', 'OAB/RJ 188.762')
ESCRITORIO_CIDADE = os.getenv('ESCRITORIO_CIDADE', 'Rio de Janeiro/RJ')
...
def gerar_peticao_inicial(classificado: dict) -> dict:
    ...
    print(f"  Contexto: {chars_usados:,} caracteres enviados para a CARNEIRO.IA")
    ...
    system_prompt = f'''Voce e CARNEIRO.IA, advogada senior com 20 anos de experiencia pratica em direito processual trabalhista, e trabalha em conjunto com o(a) advogado(a) responsavel do {ESCRITORIO_NOME}, {ESCRITORIO_ADVOGADO}, inscrito(a) na {ESCRITORIO_OAB}. Voces formam um time vitorioso.
...
RETORNE APENAS O JSON. Nao inclua texto, introducoes ou blocos ```json markdown.'''
    ...
    print("  Enviando documentos para analise da CARNEIRO.IA (Claude)...")
